...
export const calculateSecurityMetrics = (events, timeRange = 24 * 60 * 60 * 1000) => {
  ...
  const recentEvents = events.filter(e => now - new Date(e.timestamp).getTime() <= timeRange);
  ...
  metrics.successRate =
    metrics.totalEvents > 0 ? (metrics.successfulEvents / metrics.totalEvents) * 100 : 100;
  ...
  metrics.averageRiskScore =
    metrics.totalEvents > 0 ? Math.round(totalRiskScore / metrics.totalEvents) : 0;
...
export const detectThreats = (event, recentEvents = []) => {
  ...
  if (event.type === 'login' && event.success) {
    const previousLogins = recentEvents.filter(
      e =>
        e.type === 'login' &&
        e.success &&
  ...
  if (event.type === 'data_access' && event.dataType === 'sensitive') {
    const recentAccess = recentEvents.filter(
      e =>
        e.type === 'data_access' &&
        e.userId === event.userId &&
        new Date().getTime() - new Date(e.timestamp).getTime() < 60 * 60 * 1000 // 1 hour
