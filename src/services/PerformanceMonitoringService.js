  ...
  try {
    ...
    const po = new PerformanceObserver(list => {
      for (const entry of list.getEntries()) {
        callback({
          name: type,
          value: entry.startTime || entry.value || entry.duration,
          delta: 0,
          id: `v-${type}-${Date.now()}`,
          entries: [entry],
        });
...
const getFID = cb =>
  observe('first-input', m =>
    cb({ ...m, name: 'FID', value: m.entries[0]?.processingStart - m.entries[0]?.startTime })
  );
const getFCP = cb =>
  observe('paint', m => {
    if (m.entries[0]?.name === 'first-contentful-paint')
      cb({ ...m, name: 'FCP', value: m.entries[0].startTime });
  });
