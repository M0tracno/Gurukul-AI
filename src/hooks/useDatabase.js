...
export function useDatabase() {
  ...
  // Faculty operations
  const createFacultyProfile = useCallback(
    facultyData => {
  ...
  const updateFacultyProfile = useCallback(
    facultyData => {
  ...
  const assignFacultyToCourse = useCallback(
    (courseId, facultyId) => {
  ...
  const enrollStudentsInCourse = useCallback(
    (courseId, studentIds) => {
  ...
  const getAssessmentMarks = useCallback(
    (courseId, assessmentType, assessmentName) => {
  ...
  const getCourseMarks = useCallback(
    courseId => {
  ...
  const getStudentCourseMarks = useCallback(
    (studentId, courseId) => {
      return handleOperation(databaseService.marks.getStudentCourseMarks, studentId, courseId);
    },
    [handleOperation]
  );
  ...
  const getFacultyMessages = useCallback(
    (facultyId = currentUser?.uid) => {
      return handleOperation(databaseService.messages.getFacultyMessages, facultyId);
    },
    [currentUser, handleOperation]
  );

  const getParentMessages = useCallback(
    (parentId = currentUser?.uid) => {
      return handleOperation(databaseService.messages.getParentMessages, parentId);
    },
    [currentUser, handleOperation]
  );
  ...
  const markMessageAsRead = useCallback(
    messageId => {
      return handleOperation(databaseService.messages.markMessageAsRead, messageId);
    },
    [handleOperation]
  );
  ...
  const getAttendance = useCallback(
    (courseId, date) => {
      return handleOperation(databaseService.attendance.getAttendance, courseId, date);
    },
    [handleOperation]
  );
  ...
  const getCourseAttendance = useCallback(
    courseId => {
      return handleOperation(databaseService.attendance.getCourseAttendance, courseId);
    },
    [handleOperation]
  );
  ...
  const getStudentAttendance = useCallback(
    (studentId = currentUser?.uid) => {
      return handleOperation(databaseService.attendance.getStudentAttendance, studentId);
    },
    [currentUser, handleOperation]
  );
