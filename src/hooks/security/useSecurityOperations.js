...
export const useSecurityOperations = () => {
  ...
  const loadSecurityData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [userIncidents, activeThreats, metrics, userVulnerabilities, userDevices] =
        await Promise.all([
  ...
  const escalateIncident = useCallback(
    async (incidentId, escalationLevel, reason) => {
      if (!user) return false;
  ...
  const runVulnerabilityAssessment = useCallback(
    async (scope = 'user') => {
      if (!user) return null;
  ...
  return {
    ...
    hasActiveIncidents: incidents.some(
      inc => inc.status === 'open' || inc.status === 'investigating'
    ),
    hasCriticalThreats: threats.some(
