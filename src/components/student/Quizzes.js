...
function StudentQuizzes() {
        ...
        questions: [
          {
            ...
            correctAnswer: '$1$',
            explanation:
              'As $x$ approaches 0, the ratio $\\frac{\\sin(x)}{x}$ approaches 1. This is a fundamental limit in calculus.',
          },
          {
            id: 2,
            type: 'multiple_choice',
            ...
            explanation:
              'Using the power rule: $\\frac{d}{dx}[x^n] = nx^{n-1}$ and the sum rule of derivatives.',
          },
          {
            id: 3,
            type: 'multiple_choice',
          ...
          },
          {
            ...
            type: 'true_false',
            question:
              "The product rule states: If $h(x) = f(x) \\cdot g(x)$, then $h'(x) = f'(x) \\cdot g'(x)$.",
            options: ['True', 'False'],
            correctAnswer: 'False',
        ...
        questions: [
          {
            ...
            explanation:
              'The newton (N) is the SI unit of force, equal to the force needed to accelerate a mass of one kilogram by one meter per second squared.',
          },
          {
            id: 2,
            type: 'multiple_choice',
          ...
          },
          {
            ...
            explanation:
              'The total energy of an isolated system remains constant; it can only change form.',
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            explanation:
              'While identity is a theme in the novel, the primary theme explores the tension between freedom and security/control.',
          },
          {
            id: 2,
            type: 'multiple_choice',
            ...
            options: [
              ...
              'He dies',
              'He becomes the new Giver',
            ],
            correctAnswer: 'He reaches Elsewhere',
            studentAnswer: 'He reaches Elsewhere',
            explanation:
              'The ending is ambiguous, but most interpretations suggest Jonas reaches Elsewhere with Gabriel.',
          },
          {
            id: 3,
            type: 'short_answer',
            ...
            correctAnswer:
              'Release is a euphemism for euthanasia or killing members of the community.',
            studentAnswer: 'Release means killing people who break rules or are too old.',
            explanation:
              'Release is presented as an honor but is actually a form of euthanasia performed on the elderly, infants who fail to thrive, and rule-breakers.',
          },
          {
            id: 4,
            type: 'multiple_choice',
            ...
            explanation:
              'Jonas is selected to be the new Receiver of Memory, trained by the current Receiver (who becomes "The Giver").',
          },
        ],
      },
    ];
  ...
  // Filter quizzes by status  // Filter quizzes by status with array safety
  ...
  const pendingQuizzes = Array.isArray(quizzes)
    ? quizzes.filter(quiz => quiz.status === 'pending')
    : [];
              ...
              <Paper key={question.id} className={classes.question} elevation={1}>
                ...
                <div className={classes.resultItem}>
                  <Typography variant="body1">
                    Your answer:{' '}
                    {studentAnswer ? renderMathExpression(studentAnswer) : '(No answer)'}
                  </Typography>
                  {isCorrect ? (
              ...
              <Chip
                ...
                className={`${classes.chip} ${
                  quiz.status === 'upcoming'
                    ? classes.upcomingChip
                    : quiz.status === 'pending'
                      ? classes.pendingChip
                      : classes.completedChip
                }`}
              />
            </Box>
  ...
  return (
    <Container className={classes.root}>
      ...
      <Grid container spacing={3}>
        {tabValue === 0 &&
          (pendingQuizzes.length > 0 ? pendingQuizzes.map(renderQuizCard) : renderEmptyState())}

        {tabValue === 1 &&
          (upcomingQuizzes.length > 0 ? upcomingQuizzes.map(renderQuizCard) : renderEmptyState())}

        {tabValue === 2 &&
          (completedQuizzes.length > 0 ? completedQuizzes.map(renderQuizCard) : renderEmptyState())}
      </Grid>
      ...
      <Dialog
        ...
        scroll="paper"
      >
        <DialogTitle>{selectedQuiz && selectedQuiz.title}</DialogTitle>
