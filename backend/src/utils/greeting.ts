/**
 * Personalized time-of-day greeting helpers.
 *
 * The greeting shown on every dashboard is composed of a time-of-day phrase
 * (selected from the user's local hour) followed by the user's first name. The
 * logic lives in a pure, deterministic module so it can be unit- and
 * property-tested in isolation and shared as a contract between the backend and
 * the frontend (the frontend owns the rendering copy, but the phrase/name
